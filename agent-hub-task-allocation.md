...
3. 每个 Agent 使用独立 branch/worktree：
   - `agent/claude-architecture-ui`
   - `agent/codex-runtime-security`
   - `agent/opencode-adapter`
   - `agent/hermes-qa-support`
...
| next | `HUB-130`：从最新 `main` 创建 Hermes 任务分支并实现 Context/Artifact/Event 基础设施 |
...
协议冻结点为 `contracts-frozen-v1`。阶段 0 和第一波阶段 1 任务已集成到 `main`；下一执行顺序为 `HUB-130 -> 审查/合并 -> HUB-110 -> 阶段 1 CLI/Mock gate`。各 Agent 继续使用独立简报、branch 和 worktree，`main` 只用于已审查任务的最终集成。
...
| Hermes | `context/`、`storage/artifact_store.py`、`tests/fixtures/`、CI、测试报告和使用文档 |
